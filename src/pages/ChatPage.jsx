...
export default function CommunityPage() {
    ...
    const setMessagesAndCache = useCallback((updater) => {
        setMessages((prev) => {
...
// You had setMeId but it's not used later. Removed for cleanup.
            } catch (e) {
            ...
            <nav className="fixed top-0 left-0 w-full z-50 bg-[#F4F6FA] shadow-md">
                <div
                    ...
                    <div
                        ...
                        <a href="/chat"
                           className="text-[#3066BE] hover:text-[#3066BE] transition">{texts[langCode].chat}</a>
                        <a href="/companies"
                        ...
                        <div ref={topBoxRef} className="relative w-[396px]">
                            <div className="flex items-center bg-[#F4F6FA] rounded-[10px] px-4 py-2 w-[396px] h-[47px] border-none">
                                ...
                                <input
                                    ...
                                    // onKeyDown={onTopKeyDown}               // <-- Enter/↑/↓ qo‘shildi
                                    placeholder="Поиск..."
                            ...
                            {suggests.length > 0 && (
                                <div className="absolute z-50 left-0 right-0 mt-2 bg-white rounded-[10px] shadow-md border overflow-hidden">
                                    {suggests.map((u, idx) => (
                                        <button
                                            key={u.id}
                                            // onMouseDown={() => openChatWith(u.username)}   // blur bo‘lmasin
                                            className={`w-full flex items-center bg-white border-none gap-3 px-3 py-2 text-left hover:bg-gray-50 ${
                                                idx === activeIdx ? "bg-gray-50" : ""
                                            }`}
                                        >
                                            <img src={u.avatar || "/profile.png"} alt="" className="w-[34px] h-[34px] rounded-full object-cover" />
                                            <div className="flex flex-col items-start">
                                                <span className="text-sm font-semibold text-black">@{u.username}</span>
                                                <span className="text-xs text-gray-500">
                                                    {(u.full_name || u.username || "").toLowerCase()}
                                                  </span>
                                            </div>
                                        </button>
                                    ))}
            ...
            <div className="w-full max-w-[1440px] mx-auto px-4 py-6 flex gap-6 min-h-[calc(100vh-160px)] pb-[84px]">
                ...
                <div className="w-[360px] bg-white border border-black rounded-2xl p-4 flex flex-col">
                    ...
                    <div className="flex flex-col gap-4 mb-6">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                value={leftQuery}
                                onChange={(e) => setLeftQuery(e.target.value)}   // <-- faqat shu
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto flex flex-col gap-3">
                        {(Array.isArray(rooms) ? rooms : []).map((r) => (
                            ...
                            >
                                <img src={r.peer?.avatar || "/profile.png"} className="w-[46px] h-[46px] rounded-full object-cover" />
                ...
                <div className="flex-1 bg-[#F4F6FA] border border-none rounded-2xl flex flex-col">
                    ...
                    <div className="flex items-center gap-4 p-4 border-b border-none">
                        <img src={peer?.avatar || "/profile.png"} className="w-[50px] h-[50px] rounded-full object-cover" alt="" />
                        <div>
                            <h3 className="font-semibold text-lg text-black">{displayName}</h3>
                            <p className="text-sm text-gray-500">
                                {typing ? "печатает..." : (statusText || "—")}
                            </p>
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        onScroll={() => {
                            // near-bottom bo‘lsa read belgilaymiz
                    ...
                    >
                        {messages.length === 0 ? (
                            ...
                            messages.map((m, idx) => {
                                ...
                                const mine = isMineMsg(m); // << faqat shu bilan
                                    ...
                                    <div key={key} className={`${bubbleBase} ${mine ? mineCls : otherCls}`}>
                                        ...
                                        {timeStr && (
                                            <div className={`mt-1 text-[11px] flex items-center gap-1 ${mine ? "justify-end text-white/80" : "justify-start text-gray-500"}`}>
                                                <span>{timeStr}</span>
                                                {mine && (m.status === "read") && (
                                                    <img src="/double-check.png" alt="double check" className="w-[14px] h-[7px]" />
                                                )}
                                            </div>
                                        )}
                                    </div>

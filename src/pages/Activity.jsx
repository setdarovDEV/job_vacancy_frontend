                                        ...
                                        <div className="flex flex-col gap-[20px]">
                                            {texts[langCode].links.slice(4).map((link, idx) => (
                                                <a
                                                    key={idx}
                                                    href="#"
                                                    className="flex items-center gap-2 text-white hover:text-[#3066BE] text-[18px] leading-[120%] font-normal font-gilroy transition-colors durataion-300"
                                                >
                                                    <span>&gt;</span> {link}
                                                </a>
                                            ))}
            ...
            {items.map((v) => (
                <li key={v.id} className="border border-[#E5EAF2] rounded-2xl p-4 flex items-start justify-between gap-4">
                    <div className="min-w-0">
                        <div className="font-semibold text-lg leading-6 truncate">{v.title || v.name || "Vacancy"}</div>
                        <div className="text-gray-500 text-sm mt-1 line-clamp-2">
                            {(v.company?.name || v.company_name || "—") + " • " + (v.location || v.city || "Локация не указана")}
                        </div>
                        <div className="text-sm mt-1">{v.salary ? `З/п: ${formatSalary(v.salary)}` : ""}</div>
                    </div>
...
function AppliedList({ items, loading, error }) {
    ...
    return (
        <ul className="flex flex-col gap-4">
            {items.map((a) => {
                // Backend nomlariga moslashuvchan maydonlar
                ...
                return (
                    <li key={a.id} className="border border-[#E5EAF2] rounded-2xl p-4 flex items-start justify-between gap-4">
                        <div className="min-w-0">
                            <div className="font-semibold text-lg leading-6 truncate">{vacancy.title || vacancy.name || "Vacancy"}</div>
                            <div className="text-gray-500 text-sm mt-1 line-clamp-2">
...
function Empty({ text }) {
    return <div className="text-center text-gray-500 py-16 select-none">{text}</div>;
}

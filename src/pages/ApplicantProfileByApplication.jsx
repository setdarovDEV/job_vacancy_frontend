...
// import { fetchApplicantOfApplication } from "../utils/applicationsApi.js";
...
export default function ApplicantProfileByApplication() {
    ...
    useEffect(() => {
        ...
        api.get(`/api/applications/${applicationId}/applicant/`)
    ...
    if (error) return (
        <div className="p-6">
            ...
            <button className="px-4 py-2 rounded border" onClick={() => navigate(-1)}>← Orqaga</button>
        </div>
    );
        ...
        <>
            ...
            <div className="max-w-7xl w-[1176px] mx-auto px-4 py-8 mt-[-70px]">
                ...
                <div className="w-full bg-white border border-[#AEAEAE] mt-[67px] min-h-[1006px] rounded-[25px] overflow-visible">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-[15px] text-[#AEAEAE] font-medium flex items-center gap-1">
                                    {/* Joylashuv/vaqt bizda yo‘q, shunchaki position ko‘rsatamiz */}
                                    {data.position || "—"}
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="w-full min-h-full overflow-visible bg-white">
                        ...
                        <div className="flex max-w-[1176px] mx-auto w-full h-auto">
                            {/* LEFT PANEL (read-only, placeholderlar) */}
                            <div className="w-[60%] px-6 py-6 border-r border-[#AEAEAE]">
                                ...
                                <div className="pb-4 px-4 py-3 mb-[30px]">
                                    ...
                                    <p className="text-[15px] leading-[22px] text-black mt-1 font-medium">{data.work_hours_per_week || "—"}</p>
                                        ...
                                        {educations.length === 0 ? (
                                            ...
                                            educations.map((e, i) => (
                                                <div key={i} className="text-[15px] leading-[22px] text-black">
                                                    ...
                                                    <div className="text-[#9ca3af]">{e.start_year} — {e.end_year}</div>
                                                </div>
                                            ))
                                        )}
                                    </div>
                            ...
                            <div className="flex justify-center px-6 py-6 w-full">
                                <div className="w-[762px]">
                                    ...
                                    <div className="w-full bg-white border-none rounded-xl p-6">
                                        <h3 className="text-[24px] leading-[36px] font-bold text-[#000000] mb-[10px]">
                                            О себе
                                        </h3>
                                        <p className="text-[#000] text-[16px] leading-relaxed whitespace-pre-line">
                                            {data.bio || "—"}
                                        </p>
                                    </div>
                                                        ...
                                                        {p.skills_list?.length > 0 && (
                                                            <div className="flex flex-wrap gap-2 mt-2">
                                                                {p.skills_list.map((s, idx) => (
                                                                    <span key={idx} className="px-3 py-1 border text-black bg-[#D9D9D9] rounded-full text-sm">{s}</span>
                                                                ))}
                                                            </div>
                                                        )}
                                                        {p.media?.length > 0 && (
                                                                ...
                                                                {p.media.map((m, idx) => (
                                                                    ...
                                                                    >
                                                                        {m.file_type === "image" ? (
                                                                            ...
                                                                            <span className="text-sm underline text-[#3066BE]">
                                    ...
                                    <div className="w-full bg-white border-none rounded-xl p-4 mt-6">
                                        ...
                                        <div className="flex flex-wrap gap-2 justify-start mt-[21px]">
                                            {skills.length === 0 ? (
                                                <span className="bg-[#D9D9D9] text-sm text-[#AEAEAE] px-4 py-1 rounded-full">
                          —
                        </span>
                                            ) : (
                                                skills.slice(0, 20).map((skill, idx) => (
                                                    <span
                                                        key={`sk-${idx}`}
                                                        className="bg-[#D9D9D9] text-[15px] text-black px-4 py-1 rounded-full border border-gray-300"
                                                    >
                        ...
                        <div className="grid grid-cols-2 gap-6 p-6">
                            {certificates.map((c) => (
                                <div key={c.id} className="cursor-pointer border border-[#D9D9D9] rounded-[15px] overflow-hidden shadow-sm">
                                    {/* Simple: agar rasm bo'lsa rasm ko'rsatamiz; pdf bo'lsa placeholder matn */}
                ...
                <div className="w-full bg-white border border-[#AEAEAE] mt-[30px] rounded-[25px] p-6">
                    <h3 className="text-[24px] leading-[36px] font-bold text-[#000000] mb-2">
                        Опыт работы
                    </h3>
                        ...
                        <div className="flex flex-col gap-4">
                            {experiences.map((ex) => (
                                <div key={ex.id} className="border rounded-[12px] p-4">
                                    <div className="font-semibold text-black">{ex.position} @ {ex.company_name}</div>
                                    ...
                                    {ex.description && (
                                        <div className="text-[14px] text-[#4b5563] mt-1 whitespace-pre-line">{ex.description}</div>
                                    )}
                                </div>
                            ))}
                        </div>
